  ...
  return (
    ...
    >
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <Typography variant="body1" sx={{ fontWeight: 600, fontFamily: "monospace", letterSpacing: "-0.3px" }}>
          {project.name}
        </Typography>
      ...
      <Box sx={{ display: "flex", gap: 2, mt: 0.5 }}>
        <Link href={project.githubUrl} target="_blank" rel="noopener noreferrer" sx={linkSx}>
          GitHub
        ...
        {project.npmUrl && (
          <Link href={project.npmUrl} target="_blank" rel="noopener noreferrer" sx={linkSx}>
            npm
            <OpenInNewIcon sx={{ fontSize: "0.75rem" }} />
          </Link>
        )}
        {project.pluginUrl && (
          <Link href={project.pluginUrl} target="_blank" rel="noopener noreferrer" sx={linkSx}>
            Marketplace
            <OpenInNewIcon sx={{ fontSize: "0.75rem" }} />
          </Link>
        )}
...
const OSSRow = ({ contrib, recentPRs }: { contrib: OpenSourceContribution; recentPRs?: MergedPR[] }) => {
  const theme = useTheme();
  return (
    <Box
      sx={{
    ...
    >
      <Typography component="span" sx={{ color: "text.secondary", fontWeight: 600, fontSize: "1.1rem", lineHeight: 1.8, mt: 0.1 }}>
        →
      </Typography>
      <Box sx={{ flex: 1 }}>
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 1, mb: 0.5 }}>
          ...
          <Link href={contrib.githubUrl} target="_blank" rel="noopener noreferrer" sx={linkSx}>
            GitHub
            <OpenInNewIcon sx={{ fontSize: "0.75rem" }} />
          </Link>
        </Box>
          ...
          <Box sx={{ mt: 1.5, display: "flex", flexDirection: "column", gap: 0.5 }}>
            {recentPRs.slice(0, 3).map((pr) => (
              <Link
                ...
                sx={{ color: "text.secondary", textDecoration: "none", fontSize: "0.78rem", opacity: 0.75, "&:hover": { opacity: 1, textDecoration: "underline" } }}
              >
                · {pr.title}
              </Link>
            ))}
...
const ProjectsSection = () => {
  ...
  useEffect(() => {
    ...
    fetch("/api/github-activity", { signal: AbortSignal.timeout(4000) })
      ...
      .catch(() => {
        // static descriptions already render fine on their own
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <Box component="section" id="projects" sx={{ scrollMarginTop: "5rem", mb: { xs: 8, md: 10 } }}>
      <Typography component="h2" variant="h5" sx={{ fontWeight: 700, mb: 3 }}>
        projects
      </Typography>

      <Box sx={{ mb: 5 }}>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2.5, textTransform: "uppercase", letterSpacing: "1px", fontSize: "0.7rem" }}>
          my work
        </Typography>
      ...
      <Box>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2, textTransform: "uppercase", letterSpacing: "1px", fontSize: "0.7rem" }}>
          open source contributions
        </Typography>
        <Box>
          {openSourceContributions.map((contrib, idx) => (
            <OSSRow key={idx} contrib={contrib} recentPRs={livePRsByRepo[contrib.repo]} />
          ))}
        </Box>
      </Box>
    </Box>
